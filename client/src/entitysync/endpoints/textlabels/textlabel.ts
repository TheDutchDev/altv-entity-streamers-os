...
export interface ITextLabel {
    ...
    rmlElement : alt.RmlElement;
    streamed : boolean;
}

export class TextLabel implements ITextLabel {
    text : string;
    scale : number;
    color : alt.RGBA;
    position : alt.Vector3;
    ...
    constructor( data : ITextLabel, rmlDoc : alt.RmlDocument ) {
        fillObjectPartially( this, data );
        this.onCreate( rmlDoc );
    }

    onCreate( rmlDoc : alt.RmlDocument ) {
